...
for config_index, config in configurations.iterrows():
    ...
    for scenario_index, scenario in scenarios.iterrows():
        ...
        weatherfile_path = weatherfile_translation.loc[(weatherfile_translation["location"]==config['location']) & (weatherfile_translation["scenario"]==scenario['weatherfile'])]['filename'].tolist()[0]  # this weird construction is necessary to get the string instead of a pd Series
        ...
        if cooling_system == "ASHP" or cooling_system == "GSHP":
            if Gebaeude_static.cooling_elec.sum() != 0:
                annual_cooling_cop_stat[
                    config_index, scenario_index] = Gebaeude_static.monthly_cooling_demand.sum() / Gebaeude_static.cooling_elec.sum()
            else:
                annual_cooling_cop_stat[config_index, scenario_index] = 1.0

            if Gebaeude_dyn.cooling_electricity_demand.sum() !=0:
